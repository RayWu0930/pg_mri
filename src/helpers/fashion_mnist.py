...
class FashionMNISTData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        image = torch.cat([image.unsqueeze(-1), torch.zeros_like(image.unsqueeze(-1))], dim=-1)
        image_for_kspace = fastmri.ifftshift(image, dim=(0, 1))
        kspace = image_for_kspace.fft(2, normalized=False)
        
        from src.helpers.transforms import complex_abs
