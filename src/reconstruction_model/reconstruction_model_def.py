...
class UnetModel(nn.Module): # same as LOUPEUNet (which we call Unet)
    """
        PyTorch implementation of a U-Net model.
        This is based on:
            Olaf Ronneberger, Philipp Fischer, and Thomas Brox. U-net: Convolutional networks
    ...
    def forward(self, input, eps=1e-8):
        ...
        # residual connection
        # can handle both complex and real input images
        # the epsilon is required to avoid undefined gradients when the input is all zero
        int_res = ((input ** 2).sum(dim=1, keepdim=True)+eps).sqrt()

        return out_conv2 + int_res
